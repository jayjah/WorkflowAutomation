    ...
    def run(self):
        while not self.isdone:
            if self.initialized:
                ...
                density = self.device.display['density'] if self.device.display['density'] > 0 else 1
                ...
                p = call(['terminator','--new-tab', '-e', 'echo it worked'])#, '--args', 'bb.py'])
                #out = p.communicate()
                #print out
                print "DONE"
            ...
            if self.initialized and Device.disablesimlock and not self.disabledpin:
                debug_print(" Device Nummer: "+self.currentdevice+" Serialno: "+self.serialno+"")
